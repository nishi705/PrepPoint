...
public class Solution {
    public int[] solve(int[] A) {
        ...
        int n=A.length;
        int mod=1000000007;
        int min_magic=0,max_magic=0;
//For the minimum magic number, find the sum of the difference between adjacent elements in pairs of two.
        for (int i=1 ; i<n ; i+=2){
        min_magic=(min_magic+(Math.abs(A[i]-A[i-1])%mod))%mod;
        }
//For the maximum magic number, find the sum of the difference between the two elements equidistant from the front and back of the array.
        for (int i=0 ; i<n/2 ; i++){
        max_magic=(max_magic+(Math.abs(A[n/2+i]-A[i])%mod))%mod;
        }
        ans[0]=max_magic;
        ans[1]=min_magic;
        return ans;
